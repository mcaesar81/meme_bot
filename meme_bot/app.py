...
from meme_bot.utils.timefmt import LOCAL_TZ
...
def run_loop(stop_event: Event, cfg_path: str = "config.yaml") -> None:
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            if market.last_update_ts and (now.replace(tzinfo=None) - market.last_update_ts).total_seconds() > stale_limit:
            ...
            if state.active_position is None:
                allowed, reason = risk.can_open_trade(state, now.replace(tzinfo=None))
                if not allowed:
                    best_reason = reason
                elif not filtered:
                    ...
                    if last_trade_for_token:
                        ...
                        if now.replace(tzinfo=None) - last_trade_dt < timedelta(minutes=cooldown_min):
                    ...
                    if reference_price <= 0:
                        ...
                        state.active_position = asdict(
                            Position(
                                ...
                                last_scale_in_at=now.replace(tzinfo=None),
                            )
                        )
                        state.trades_today += 1
                        state.last_trade_ts_iso = now.isoformat()
                        token_last_trade_iso[best.address] = now.replace(tzinfo=None).isoformat()
                        logger.trade(result)
                    else:
                ...
                scale = strategy.maybe_scale_in_size(pos, now.replace(tzinfo=None), unrealized)
                if scale > 0:
                    ...
                    if risk.slippage_ok(buy_px, px):
                        ...
                        pos.size_usd += result.filled_usd
                        pos.last_scale_in_at = now.replace(tzinfo=None)
                        logger.trade(result)

                exit_now, reason = strategy.should_exit(pos, now.replace(tzinfo=None), unrealized)
                if exit_now or pos.size_usd <= 0:
                    ...
                    state.last_trade_ts_iso = now.isoformat()
                    token_last_trade_iso[pos.token_address] = now.replace(tzinfo=None).isoformat()
                    logger.trade(result)
                    logger.event(
                        "position_closed",
                        {
