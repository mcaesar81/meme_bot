...
from meme_bot.utils.timefmt import iso_local, iso_utc
...
def get_status() -> dict[str, Any]:
    ...
    status["server_ts_local"] = iso_local()
    status["server_ts_utc"] = iso_utc()
    return status
...
<head>
  ...
  <style>
    ...
    .trade-win { color:#22c55e; font-weight:700; }
    .trade-loss { color:#ef4444; font-weight:700; }
    .trade-flat { color:#9ca3af; font-weight:600; }
    .old-trade { opacity:0.6; }
...
<body>
  ...
  <div id=\"runner\"></div>
  <div id=\"lastUpdate\" class=\"muted\">Last update: -</div>
...
function tradeClass(item, idx, total) {
  ...
  if (!Number.isNaN(pnl)) {
    ...
    else if (pnl < -0.005) cls = 'trade-loss';
    else cls = 'trade-flat';
  } else {
    const side = String(item.side || '').toLowerCase();
    cls = side === 'buy' ? 'trade-buy' : (side === 'sell' ? 'trade-sell' : '');
  }
  if (total - idx > 10) cls += ' old-trade';
  return cls.trim();
}
...
function logText(item) {
  if (item.event_type) {
    ...
    if (item.event_type === 'no_entry') {
      return `${ts} [no_entry] ${token} reason=${p.reason} liq=${p.candidate_liquidity_usd ?? '-'} vol5m=${p.candidate_vol_5m_usd ?? '-'} dex=${p.dex_id ?? '-'} pair=${p.pair_address ?? '-'}`;
    }
...
function renderLogs() {
  ...
  pane.innerHTML = items.map((item, idx) => {
    const cls = activeTab === 'events' ? eventClass(item) : tradeClass(item, idx, items.length);
